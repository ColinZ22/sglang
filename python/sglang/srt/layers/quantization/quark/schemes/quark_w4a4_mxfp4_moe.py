...
class QuarkW4A4MXFp4MoE(QuarkMoEScheme):
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if self.dequantization_config is not None:
            ...
            elif isinstance(self.dequantization_config, Nvfp4SourceConfig):
    ...
    def _create_weights_from_nvfp4_moe(
        ...
        *,
        ...
        has_scale_2 = self.dequantization_config.has_per_tensor_scale
        layer._nvfp4_loaded_numel = 0
        layer._nvfp4_materialized = False
        # Pin to an explicit ordinal; see note in linear scheme.
    ...
    def get_online_nvfp4_to_mxfp4_weight_loader(self, layer, original_weight_loader):
        """NVFP4 MoE loader. Mirrors get_online_fp8_to_mxfp4_weight_loader:
        materialize on first touch, then expert-wise dequant+requant once all
        source bytes are in place."""
        has_scale_2 = self.dequantization_config.has_per_tensor_scale
        bulk_names = ["w13_weight", "w2_weight", "w13_weight_scale", "w2_weight_scale"]
        scale2_names = (
            ["w13_weight_scale_2", "w2_weight_scale_2"] if has_scale_2 else []
        )
        ...
        def loader(param, loaded_weight, weight_name, shard_id, expert_id):
            ...
            with layer._nvfp4_loading_lock:
                ...
                if layer._nvfp4_loaded_numel == total:
                    self._requantize_nvfp4_to_mxfp4(layer, "w13", has_scale_2)
                    self._requantize_nvfp4_to_mxfp4(layer, "w2", has_scale_2)
    ...
    def _requantize_nvfp4_to_mxfp4(self, layer, prefix, has_scale_2):
        ...
        w_s2 = getattr(layer, f"{prefix}_weight_scale_2") if has_scale_2 else None
        qws, qss = [], []
