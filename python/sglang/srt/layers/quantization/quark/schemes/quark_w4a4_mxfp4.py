...
class QuarkW4A4MXFP4(QuarkLinearScheme):
    ...
    def _create_weights_from_nvfp4(
        ...
        # Pin to an explicit ordinal: torch.get_default_device() may return
        # `cuda` (no index), which breaks the `current_device() == idx` assert
        # in the loader.
        ...
        with torch.device("meta"):
            ...
            # to write into. The no-op matches the LinearMethodBase contract:
            # (param, loaded_weight, shard_id=None).
    ...
    def get_online_nvfp4_to_mxfp4_weight_loader(
        ...
        """NVFP4 -> MXFP4 loader. Mirrors get_online_fp8_to_mxfp4_weight_loader:
        materialize on first touch, dequantize+requantize once all source bytes
        are in place."""
        has_scale_2 = self.dequantization_config.has_per_tensor_scale
        ...
        def loader(param, loaded_weight, shard_id=None):
            ...
            with layer._nvfp4_loading_lock:
                ...
                target = layer.weight.numel() + layer.weight_scale.numel()
                if has_scale_2:
                    target += layer.weight_scale_2.numel()
